...
const AnswerButton = ({children, index, question, setAnswered, score, setAnswerText, setIsCorrect, correctTries})=>{
    ...
    return (
        <button id={index} onClick={handleClick} className="neo-brutalism-white-button bg-white hover:bg-amber-50 hover:scale-110 active:bg-amber-100 active:scale-95 neo-btn-magic-question hover:cursor-pointer sm:w-1/2">{children}</button>
    )
}
...
const MagicButton = ({onClick, className, children}) => {
    ...
    return (
        <button onClick={onClick} className={`w-[15%] neo-brutalism-white-button bg-white hover:bg-amber-50 hover:scale-110 active:bg-amber-100 active:scale-95 neo-btn-magic-question hover:cursor-pointer z-20 -mt-5 ${className}`}>{children}</button>
    )
}
